...
- 新增：「通讯录与拨号」详情页「拨号盘与列表」适配开关——开启后清除联系人列表的纯色底（深色下的黑色横条），并把拨号盘键盘面板设为半透明，让背景透出。
- 新增：键盘面板不透明度滑块（0–100%，默认 60%），仅在适配开关开启时显示；面板背景为 9-patch 图，改用整体透明度而非清除背景，保留圆角与按键可读性。
- 说明：拨号盘键盘由点击后异步展开，通过常驻布局监听在其出现时补设透明度，保证展开即生效。

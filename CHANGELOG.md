...
- 修复系统桌面二级设置页面未安装专用生命周期 Hook 的问题。
- 将桌面设置页面 Hook 与系统设置页面 Hook 分离，避免使用错误的 ClassLoader 和宿主类。
- 增加桌面设置 Activity 候选入口及生命周期诊断日志，便于确认新版桌面运行时是否命中。
- 测试重点为系统桌面设置中的二级菜单背景挂载、页面进入退出和桌面进程注入状态。

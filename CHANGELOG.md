...
- 还原桌面设置背景逻辑至 test5 可用状态：恢复 `home` 分支的短路判断，回退 test6 为适配 Flutter 桌面所做的改动。
- 说明：当前 HyperOS 桌面二级设置页由 Flutter/Rust 渲染，模块的 View 树背景注入对其无效；本次仅确保旧版桌面设置背景原本能生效的路径不被破坏。
- 仅回退桌面设置这一项，不影响设置搜索遮罩透明化等其它已生效逻辑。
- 测试重点为桌面设置页面背景、页面进入退出以及现有设置搜索功能。

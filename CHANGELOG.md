...
- 修复系统设置中的桌面设置二级页面背景未透出的问题。
- 恢复通过 `com.android.settings.MiuiSettings` 和 `SettingsFragment` 处理桌面设置页面。
- 桌面设置页面使用 MIUIX 顶层宿主并清理页面表面背景，保留原有卡片和控件背景。
- 撤销无效的 `com.miui.home` 专用生命周期 Hook。
- 测试重点为桌面设置二级页面背景、页面进入退出以及现有设置搜索功能。

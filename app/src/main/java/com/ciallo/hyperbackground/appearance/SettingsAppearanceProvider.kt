...
class SettingsAppearanceProvider : ContentProvider() {
    ...
    override fun query(
        ...
        val values = mapOf(
            ...
            COLUMN_COS_CARD_TITLE to prefs.getString("cos_card_title", COS_CARD_DEFAULT_TITLE).orEmpty().ifBlank { COS_CARD_DEFAULT_TITLE },
            COLUMN_COS_CARD_SUBTITLE to prefs.getString("cos_card_subtitle", COS_CARD_DEFAULT_SUBTITLE).orEmpty().ifBlank { COS_CARD_DEFAULT_SUBTITLE },
            COLUMN_COS_CARD_SIGNATURE to prefs.getString("cos_card_signature", COS_CARD_DEFAULT_SIGNATURE).orEmpty().ifBlank { COS_CARD_DEFAULT_SIGNATURE },
        )
    ...
    private fun enabled(prefs: android.content.SharedPreferences, slot: String) = when (slot) {
        ...
        APPEARANCE_SLOT_STYLE2_DEVICE_IMAGE, APPEARANCE_SLOT_STYLE2_CUSTOM_DEVICE_LOGO -> prefs.getInt("device_interface_style", DEVICE_INTERFACE_STYLE_SYSTEM) == DEVICE_INTERFACE_STYLE_THREE
        APPEARANCE_SLOT_STYLE2_UPDATE_BACKGROUND -> prefs.getInt("device_interface_style", DEVICE_INTERFACE_STYLE_SYSTEM) == DEVICE_INTERFACE_STYLE_THREE
        else -> false
    }
